#!/bin/bash
# DoubleLight — AI Risk Analysis Component
# cd ~/doublelight && bash ai-risk-analysis.sh

# ============================================================
# 1. src/app/components/AIRiskPanel.js
# ============================================================
cat << 'AIRISKEOF' > src/app/components/AIRiskPanel.js
"use client";
...
echo "✅ AIRiskPanel.js created"

# ============================================================
# 2. Patch page.js — add AIRiskPanel import + usage
# ============================================================

# Backup
cp src/app/page.js src/app/page.js.bak
...
# 1. Add AIRiskPanel import after SwapInfo import
old_import = 'import { InfoRow } from "./components/SwapInfo";'
new_import = '''import { InfoRow } from "./components/SwapInfo";
import AIRiskPanel from "./components/AIRiskPanel";'''
content = content.replace(old_import, new_import)

# 2. Add AIRiskPanel after SwapInfo in swap tab
# Find the SwapInfo line and add AIRiskPanel after it
old_swapinfo = '{fromAmt && <SwapInfo from={fromTokenLive} to={toTokenLive} shielded={shielded} />}'
new_swapinfo = '''{fromAmt && <SwapInfo from={fromTokenLive} to={toTokenLive} shielded={shielded} />}

              {/* AI Risk Analysis */}
              <AIRiskPanel fromToken={fromTokenLive} toToken={toTokenLive} amount={fromAmt} />'''
content = content.replace(old_swapinfo, new_swapinfo)

with open("src/app/page.js", "w") as f:
    f.write(content)

print("✅ page.js patched — AIRiskPanel added")
PYEOF

echo ""
echo "✅ All done! Files:"
echo "  - src/app/components/AIRiskPanel.js (NEW)"
echo "  - src/app/page.js (PATCHED)"
echo "  - src/app/page.js.bak (BACKUP)"
echo ""
echo "Next:"
echo "  npx next build"
echo "  git add . && git commit -m 'feat: AI risk analysis panel for swap' && git push origin main"
